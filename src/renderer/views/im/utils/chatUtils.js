...
export function fetchPost(url, formData, resultFun, self) {
  tokenFetch(url, formData, self)
      ...
      if (response.status === 401) {
        ...
        flushToken()
          .then(json => {
            ...
            //重新请求
            fetchPost(url, formData,resultFun, self)
          })
          .catch(() => {
            self.$Message.error("与服务器通讯错误，将退出登录");
            logout(self);
          });
    ...
    .catch(error => {
      self.$Message.error("与服务器通讯失败，请退出重新登录");
    });
}
