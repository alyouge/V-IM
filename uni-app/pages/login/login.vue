<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="username" type="text" maxlength="11" placeholder="手机号码"></wInput>
				<wInput v-model="password" type="password" maxlength="11" placeholder="密码"></wInput>
				<wInput v-model="host" type="text" maxlength="15" placeholder="主机"></wInput>
			</view>
...
<script>
	...
	import ChatUtils from '../../components/conf/ChatUtils.js' //ChatUtils
	import conf from '../../components/conf/index.js' //conf
	...
	export default {
		...
		mounted() {
			let self = this;
			self.host = uni.getStorageSync('host')?uni.getStorageSync('host'):'101.200.151.183';
			//this.isLogin();
		},
		methods: {
			isLogin() {
			...
			login: function() {
				let self = this;
				uni.setStorageSync('host',self.host);
				console.log('开始登录。。。')
				console.log(conf.getTokenUrl())
				uni.request({
					url: conf.getTokenUrl(), //仅为示例，并非真实接口地址。
					data: {
						client_id: 'v-client',
					...
					success: (res) => {
						...
						if(token.error && token.error === 'invalid_grant'){
							...
							self.initUserInfo(token.access_token);
							uni.reLaunch({
			...
			//初始化用户的好用信息
			initUserInfo: function(access_token) {
				let self = this;
				uni.request({
					url: conf.getInitUrl(), //仅为示例，并非真实接口地址。
					data: {
						token:access_token
					},
					fail: (error) => {
						...
						uni.showToast({
							icon: 'none',
							position: 'bottom',
							title: '用户信息初始化失败：' + error.errMsg
						});
					},
					success: (res) => {
						let userInfo = res.data;
						if(token.error && token.error === 'invalid_token'){
							//用户信息初始化失败
							uni.showToast({
								icon: 'none',
								position: 'bottom',
